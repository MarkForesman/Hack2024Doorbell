...
class OrchestratorServer:
        ...
        try:
            while self.running:
                ...
                while '\n' in buffer:
                    ...
                    if message:
                        dev_id = self.process_message(message, client_socket, address, device_id)
                        # Update device_id if this was a registration
                        if not device_id and dev_id:
                            device_id = dev_id
    ...
    def process_message(self, message: str, client_socket: socket.socket, address, device_id):
        """Process incoming message from client, return device_id if registered"""
        try:
    ...
    def handle_registration(self, data: dict, client_socket: socket.socket, address):
        """Register a new device, return device_id"""
    ...
    def handle_button_press(self, data: dict):
        ...
        logger.info(f"Button press: {device_id} ({device_type}) - Button {button} - {location}")
